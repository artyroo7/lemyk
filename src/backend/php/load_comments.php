<?php
    ...
    $sql = "SELECT thread_comments_number FROM thread_info WHERE thread_id = ?";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("s", $id);
    $stmt->execute();
    $comments_result = $stmt->get_result();
    $comment = $comments_result->fetch_assoc();
    $comments_number = $comment["thread_comments_number"];

    // Fetch the comments of this thread
    $sql = "SELECT * FROM thread_comment WHERE thread_id = ?";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("s", $id);
    ...
    echo '<h4 class="pb-3 fs-20">'.$comments_number.' коментарі</h4>
